...
public class AlternativeFireMethods {
    ...
    @SubscribeEvent
    public void campfireAddfuel(PlayerInteractEvent.RightClickBlock event) {
        PlayerEntity player = event.getPlayer();
        BlockPos pos = event.getPos();
        World world = event.getWorld();
        BlockState blockState = world.getBlockState(pos);
        ...
        int rawBurnTime = ForgeHooks.getBurnTime(stack);

        if (world.isRemote && CampfireBlock.isLit(blockState) && rawBurnTime > 0)
            world.addParticle(ParticleTypes.SMOKE, pos.getX(), pos.getY() + 0.25D, pos.getZ(), 0, 0.05D, 0);

        if (rawBurnTime > 0) {
            if (((ICampfireExtra) world.getTileEntity(pos)).getLifeTime() != -1337) {
                if (!world.isRemote) {
                    int burnTime = rawBurnTime * CampfireOverhaulConfig.CAMPFIRE_FUEL_MULTIPLIER.get();
                    CampfireTileEntity tileEntity = (CampfireTileEntity) world.getTileEntity(pos);
                    ICampfireExtra lifeTime = ((ICampfireExtra) tileEntity);

                    if (lifeTime.getLifeTime() < (CampfireHelper.isSoul(blockState) ? CampfireOverhaulConfig.SOUL_CAMPFIRE_MAX_LIFE_TIME.get() : CampfireOverhaulConfig.CAMPFIRE_MAX_LIFE_TIME.get()) && lifeTime.getLifeTime() >= 0) {
